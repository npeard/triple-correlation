...
def Figure_1():
    ...
    q1x = np.linspace((-0.5) * (np.max(X) + 2),
                      (0.0625) * (np.max(X) + 2), 100)
    ...
    ax1.text(-0.2 * (np.max(X) + 2), (np.max(X) + 2), -0.15 *
             (np.max(X) + 2), r'$\vec{q}_3$', (1, 0, -0.5), zorder=1000)
...
def BiSpectrumClosure():
    ...
    masked_closurePhase = np.ma.masked_where(
        fluo.weights_2d == 0, closurePhase)
...
def Figure_2():
    ...
    s.set_xticks([0, 5])
    s.set_yticks([-3 * np.pi, -2 * np.pi, -np.pi,
...
def Figure_S2():
    ...
    box_extent = fluo.kmax
    im = ax1.imshow(fluo.get_incoh_intens(), cmap='coolwarm', origin='lower',
                    vmin=0, vmax=1, extent=[-box_extent, box_extent, -box_extent, box_extent])
    ax1.text(0.05, 0.95, 'A', transform=ax1.transAxes,
             fontsize=22, fontweight='bold', va='top', c='white')
    ...
    num_shots = 1000
    box_extent = 2 * fluo.kmax
    measured = fluo.marginalize_g2(
        num_shots=num_shots) - 1 + 1. / fluo.num_atoms
    im = ax2.imshow(measured, cmap='coolwarm', origin='lower', vmin=0,
                    vmax=1, extent=[-box_extent, box_extent, -box_extent, box_extent])
    ...
    box_extent = np.max(np.fft.fftshift(np.fft.fftfreq(
        2 * fluo.num_pix, d=2 * (2 * fluo.kmax) / (2 * fluo.num_pix))))
    # IFT = Autocorrelation enhanced
    measured_ift = np.abs(
        np.fft.fftshift(
            np.fft.ifftn(
                np.fft.fftshift(measured))))
    measured_ift = measured_ift / np.max(measured_ift)
    im = ax3.imshow(measured_ift, cmap='binary', origin='lower', vmin=0,
                    vmax=0.1, extent=[-box_extent, box_extent, -box_extent, box_extent])
    ax3.text(0.05, 0.95, 'E', transform=ax3.transAxes,
             fontsize=22, fontweight='bold', va='top', c='black')
    ...
    im = ax4.imshow(incoh_sum / num_shots, cmap='coolwarm', origin='lower',
                    vmin=0, vmax=1, extent=[-box_extent, box_extent, -box_extent, box_extent])
    ...
    im = ax5.imshow(true, cmap='coolwarm', origin='lower', vmin=0,
                    vmax=1, extent=[-box_extent, box_extent, -box_extent, box_extent])
    ...
    im = ax6.imshow(true_ift, cmap='binary', origin='lower', vmin=0,
                    vmax=0.1, extent=[-box_extent, box_extent, -box_extent, box_extent])
...
def Figure_S3():
    ...
    ax1.imshow(fluo.object, extent=[-box_extent, box_extent, -
               box_extent, box_extent], origin='lower', cmap='gray')
    ax1.scatter(fluo.coords[1, :], fluo.coords[0, :],
    ...
    ax4.imshow(fluo.object_double,
               ...
               origin='lower',
               cmap='gray')
    ax4.scatter(fluo.coords[1, :], fluo.coords[0, :],
                facecolors='none', edgecolors='cyan')
    # ax4.get_xaxis().set_visible(False)
    # ax4.get_yaxis().set_visible(False)
...
def Figure_S5():
    ...
    ramped = unwrap_phase(fluo.coh_phase_double +
                          np.linspace(0, 50, len(fluo.coh_phase_double)))
    flipped = unwrap_phase(-fluo.coh_phase_double)
    rampFlipped = unwrap_phase(-fluo.coh_phase_double +
                               np.linspace(0, -25, len(fluo.coh_phase_double)))
    ...
    obj_Phase = np.fft.fftshift(obj_Phase)
    obj_scaled_x = np.fft.fftshift(np.fft.fftfreq(
        zero_pad, d=2 * 2 * fluo.kmax / (2 * fluo.num_pix)))
    # Harmonic inversion
    ...
    harm_x = inversion.frequency[inversion.amplitude > 0.1][1:] * \
        np.sign(inversion.phase[inversion.amplitude > 0.1][1:])
    print("Truth", fluo.coords)
    # frequencies
    print("Frequencies", inversion.frequency[inversion.amplitude > 0.1])
    # decay rates
    ...
    signal = np.real(signal)
    inversion = harminv.invert(signal[fluo.num_pix:],
                               fmin=0,
                               fmax=1,
...
def Figure_S4():
    ...
    ax1.imshow(unwrap_phase(fluo.coh_phase_double), origin='lower',
               extent=[-box_extent, box_extent, -box_extent, box_extent])
    # ax1.get_xaxis().set_visible(False)
    # ax1.get_yaxis().set_visible(False)
    ...
    ax2.imshow(unwrap_phase(fluo_coarse.coh_phase_double), origin='lower',
               extent=[-box_extent, box_extent, -box_extent, box_extent])
    ...
    obj_Phase = np.fft.fftshift(obj_Phase)
    obj_scaled_x = np.fft.fftshift(np.fft.fftfreq(
        len(obj_Phase), d=2 * 2 * fluo.kmax / (2 * fluo.num_pix)))
    box_extent = np.max(
        np.fft.fftshift(
            np.fft.fftfreq(
                2 *
    ...
    ax3.imshow(np.abs(obj_Phase), origin='lower',
               extent=[-box_extent, box_extent, -box_extent, box_extent], cmap='gray')
    ax3.set_xlim([-1, 1])
    ax3.set_ylim([-1, 1])
    # ax3.get_xaxis().set_visible(False)
    # ax3.get_yaxis().set_visible(False)
    ...
    ax4.imshow(np.abs(obj_Phase), origin='lower',
               extent=[-box_extent, box_extent, -box_extent, box_extent], cmap='gray')
    ax4.set_xlim([-1, 1])
    ax4.set_ylim([-1, 1])
    # ax4.get_xaxis().set_visible(False)
