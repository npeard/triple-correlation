...
class Plot1D:
    def __init__(self, num_atoms=5, num_pix=201, kmax=10):
        ...
        self.kmax = kmax
        self.fluo = Fluorescence1D(
            kmax=self.kmax, num_pix=self.num_pix, num_atoms=self.num_atoms
        )
    ...
    def plot_Object(self):
        ...
        scaled_x = np.fft.fftshift(
            np.fft.fftfreq(self.fluo.num_pix, d=2 * self.fluo.kmax / self.fluo.num_pix)
        )
    ...
    def plot_Intensities(self, num_shots=10000):
        ...
        measured = g2 - 1 + 1.0 / self.fluo.num_atoms
        ...
        P.plot(
            self.fluo.k_pix,
            incoh_sum / num_shots * self.fluo.num_atoms,
            label='Summed Speckle Pattern',
        )
    ...
    def plot_ClosurePhase(self, num_shots=10000):
        ...
        im = s.imshow(
            np.arccos(self.fluo.cosPhi_from_data(num_shots=num_shots)), origin='lower'
        )
        s.set_title('Phase from Data')
        P.colorbar(im, ax=s)

        s = fig.add_subplot(122)
        im = s.imshow(np.arccos(self.fluo.cosPhi_from_structure()), origin='lower')
    ...
    def plot_cosPhi(self, num_shots=10000):
        ...
        im = s.imshow(cosPhi_from_data - cosPhi_from_data_symmetrized, origin='lower')
        s.set_title('AntiSymmetry from Data')
        P.colorbar(im, ax=s)

        s = fig.add_subplot(224)
        im = s.imshow(cosPhi_from_phase - cosPhi_from_phase_symmetrized, origin='lower')
    ...
    def plot_simple_PhiSolve(self, num_shots=1000):
        ...
        P.plot(
            np.linspace(0, len(real_phase), len(real_phase)), real_phase, label='Exact'
        )
        P.plot(
            np.linspace(0, len(solved), len(solved)),
            solved,
            'o--',
            label=r'$\Phi = |\Phi|$',
        )
    ...
    def plot_PhiSolver(self, num_shots=10000):
        ...
        P.plot(
            np.linspace(0, len(real_phase), len(real_phase)),
            np.cos(plot_real_phase - plot_solved),
            label='cos(Diff)',
        )
        P.legend()
        P.tight_layout()
